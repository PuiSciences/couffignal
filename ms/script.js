...
// ============================================================
// Liste copiable des tickets (maintenance), tous statuts confondus,
// groupée par salle — pour transmission facile (mail, message...)
// ============================================================
function generateTicketList() {
    const wrap = document.getElementById("ticketListWrap");
    const output = document.getElementById("ticketListOutput");

    const dateStr = new Date().toLocaleDateString("fr-FR");
    let lines = [`Liste des tickets (maintenance) - Météo des salles - ${dateStr}`];
    lines.push("=".repeat(lines[0].length));
    lines.push("");

    let total = 0;
    salles.forEach(salle => {
        if (salle.tickets.length === 0) return;
        lines.push(salle.id);
        salle.tickets.forEach(ticket => {
            total++;
            lines.push(`  - [${ticket.status}] ${ticket.description} (priorité : ${getPriorityText(ticket.priority)}, ${ticket.date})`);
        });
        lines.push("");
    });

    if (total === 0) {
        lines.push("Aucun ticket enregistré.");
    }
